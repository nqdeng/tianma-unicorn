...
var	SourceJIT = Source.extend({
		...
		load: function (url, callback) {
			...
			if (type === '.js') {
				globalFile.push((base[0] === '') ?
					'js/5v/lib/ae/debug/global-img-server.js' :
					'lib/ae/debug/global-img-server.js');
			}

			this._refresh(base, pathnames.concat(globalFile), function (filtered) {
				callback(self._stamp(pathnames), function (callback) {
					self._expand(filtered, type).forEach(function (pathname, index) {
						files[index] = {
							pathname: pathname,
							data: cache[pathname]
						};
