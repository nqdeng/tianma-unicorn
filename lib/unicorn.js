...
var PATTERN_IMAGE_URL = /url\s*\(\s*(["']?)([^\)]+?\.(?:png|gif|jpg|jpeg)(?:\?[^\)]+?)?)\1\s*\)/gi,
	...
	Unicorn = util.inherit(Object, {
		 ...
		 * @param filter {string}
		 */
		_getVersion: function (fnName, ims, filter) {
			var config = this._config,
				source = this._source,
			...
			source[fnName](function (mtime, data) {
				var headers = {
						'expires': new Date(now.getTime() + 1000 * expires).toGMTString(),
						'cache-control': 'max-age=' + expires,
		...
		main: function (request, response) {
			var context = this.context,
				...
				ims = new Date(request.head('if-modified-since') || 0),
				url;

			if (pathname === '/version') {
				this._getVersion('version', ims, request.query && request.query.filter);
			} else if (pathname === '/versionData.htm') {
				this._getVersion('versionData', ims);
